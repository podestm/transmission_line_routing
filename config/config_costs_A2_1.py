# ===========================================================
# Konfigurace vrstev hybridniho cost fieldu
# ===========================================================

# === Parametry grafu ===
GRAPH_CONFIG = {
    "crs": 5514,
}

# === Parametry vysek a sklonu ===
ELEVATION_CONFIG = {
    "contour_path": "data/raw/vrstevnice.gpkg",
    "height_field": None,
    "height_field_candidates": ["VYSKA", "vyska__m_", "vyska_m", "height", "elevation", "z"],
    "max_slope_deg": 45.0,
    "penalty_from_deg": 15.0,
    "penalty_max_multiplier": 5.0,
}

# === Jednotna konfigurace vrstev (cost + pravidla) ===
#
# Poznamka k inward gradientu:
# - plati jen pro vybrane chranene vrstvy
# - gradient je uvnitr polygonu (od okraje dovnitr), ne ven
# - "distance" = 0 znamena gradient vypnuty (cele uzemi je exkluze)
#
# Vrstvy jsou rozdeleny do 5 skupin:
#   1) Pruchozi plochy (nizky cost)
#   2) Pruchozi liniove prvky s buffer exkluzi (silnice, zeleznice)
#   3) Vodni plochy (vyssi cost, buffer exkluze)
#   4) Exkluzni zastavba a infrastruktura (cost=inf)
#   5) Chranena uzemi (cost=inf, volitelny inward gradient)
#
# Poznamka k outward gradientu:
# - plati jen pro skupinu 4 (zastavba/infrastruktura)
# - gradient je vne polygonu (od okraje smerem ven)
# - penalizace klesa se vzdalenosti od prekazky

LAYER_DEFINITIONS = {

    # -----------------------------------------------------------
    # 1) PRUCHOZI PLOCHY â€” nizky cost, lze umistit uzly
    # -----------------------------------------------------------
    "OrnaPudaAOstatniDaleNespecifikovanePlochy": {
        "cost": 1.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "TrvalyTravniPorost": {
        "cost": 1.5,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "LesniPudaSKrovinatymPorostem": {
        "cost": 1.8,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "OstatniPlochaVSidlech": {
        "cost": 10.0,
        "can_place_nodes": True,
        "buffer_exclusion": 50,
    },
    "ParkovisteOdpocivka": {
        "cost": 5.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "OkrasnaZahradaPark": {
        "cost": 10.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "LesniPudaSKosodrevinou": {
        "cost": 2.6,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "Vinice": {
        "cost": 3.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "LesniPudaSeStromy": {
        "cost": 4.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "BazinaMocal": {
        "cost": 5.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "Raseliniste": {
        "cost": 5.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "SesuvPudySut": {
        "cost": 6.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },
    "SkalniUtvary": {
        "cost": 8.0,
        "can_place_nodes": True,
        "buffer_exclusion": 0,
    },

    # -----------------------------------------------------------
    # 2) LINIOVE PRVKY S BUFFER EXKLUZI â€” pruchozi, ale nelze
    #    umistit uzly; buffer kolem osy vytvari exkluzni zonu
    # -----------------------------------------------------------
    "Ulice": {
        "cost": 5.0,
        "can_place_nodes": False,
        "buffer_exclusion": 20,
        "can_cross": True,
    },
    "SilniceDalnice": {
        "cost": 10.0,
        "can_place_nodes": False,
        "buffer_exclusion": 20,
        "can_cross": True,
    },
    "ZeleznicniTrat": {
        "cost": 10.0,
        "can_place_nodes": False,
        "buffer_exclusion": 20,
        "can_cross": True,
    },

    # -----------------------------------------------------------
    # 3) VODNI PLOCHY â€” vyssi cost, buffer exkluze
    # -----------------------------------------------------------
    "VodniPlocha": {
        "cost": 10.0,
        "can_place_nodes": False,
        "buffer_exclusion": 20,
        "can_cross": True,
    },

    # -----------------------------------------------------------
    # 4) EXKLUZNI ZASTAVBA A INFRASTRUKTURA â€” cost=inf, nelze
    #    krizovat ani umistit uzly; outward gradient buffer
    # -----------------------------------------------------------
    "ArealUceloveZastavby": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "BudovaJednotlivaNeboBlokBudov": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "OvocnySadZahrada": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "Letiste": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "RozvodnaTransformovna": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "Elektrarna": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "Skladka": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "PovrchovaTezbaLom": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "Tribuna": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "Hrad": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },
    "Zamek": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "outward_gradient": {
            "distance": 500,
            "cost_outer": 2.0,
            "cost_inner": 9.0,
        },
    },

    # -----------------------------------------------------------
    # 5) CHRANENA UZEMI â€” cost=inf, volitelny inward gradient
    #    (gradient = postupny prechod od okraje dovnitr)
    # -----------------------------------------------------------
    "MaloplosneZvlasteChraneneUzemi": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
    },
    "VelkoplosneZvlasteChraneneUzemi": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "inward_gradient": {
            "distance": 0,
            "cost_outer": 1.0,
            "cost_inner": 4.0,
        },
    },
    "EvropskyVyznamnaLokalita": {
        "cost": float("inf"),
        "can_place_nodes": False,
        "buffer_exclusion": 0,
        "can_cross": False,
        "inward_gradient": {
            "distance": 0,
            "cost_outer": 1.0,
            "cost_inner": 4.0,
        },
    },
}

# === Odvozene mapy pro zpetnou kompatibilitu ===
LAYER_COSTS = {name: cfg["cost"] for name, cfg in LAYER_DEFINITIONS.items()}

LAYER_RULES = {
    name: {k: v for k, v in cfg.items() if k != "cost"}
    for name, cfg in LAYER_DEFINITIONS.items()
}

